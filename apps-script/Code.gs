...
const HEADERS=['ID','Tipo','Data UTC','Quantidade ETH','Preço USD','Taxa','Moeda taxa','BNB USD','Compra origem','Nota','Lucro realizado USD'];
function setup(){
...
function refreshSummary(trades){const r=calculate(trades),p=periods(r.sales);SpreadsheetApp.getActiveSpreadsheet().getSheetByName('ETH_Resumo').getRange(2,2,9,1).setValues([[p.total],[p.day],[p.week],[p.month],[p.year],[r.qty],[r.cost],[r.fees],[new Date().toISOString()]]);}
